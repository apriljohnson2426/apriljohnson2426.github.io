var c;
var t = {f: 0, fps: 60, fpsInterval: 0, now: 0, then: 0, elapsed: 0, stop: false, realFPS: 0, modeinit: 0}
...
var shots = []
var enemies = []
var info = {score: 0, highscore: 1000, shotsTaken: 0, shotsHit: 0, accuracy: 100}
var particles = [[],[],[],[],[],[],[]]
var mode = "start"
var font1 = "28px serif"
var font2 = "16px serif"
var font3 = "12px serif"
var font4 = "16px monospace"
var measureFrame = 0
var frameTimes = []
var canvaswidth = 240
...
class enemy {
	constructor(x,y,w,h,xv,yv,HP,MaxHP,lifetime,life,fireRate) {
		this.x = x
		this.y = y
		this.w = w
		this.h = h
...
window.onresize = function() {
	...
	if (ih > iw * height/width) {
		c.c.style.height = iw / width * height + "px"
	} else {
		c.c.style.height = ih + "px"
		c.c.style.position = "absolute"
		c.c.style.left = (iw - ih * width / height) * 0.5 + "px"
	}
}
...
function measureRealFPS() {
	frameTimes[measureFrame] = t.elapsed
	measureFrame++
	if (measureFrame>7) {measureFrame = 0}
	var total = 0
	for (i=0;i<frameTimes.length;i++) {
		total += frameTimes[i]
	}
	t.realFPS = 1000 / (total/frameTimes.length)
}
...
function playUpdate() {
	...
	enemyTimeout()
	enemyHPCheck()
	if (t.f%(t.fps*0.5)==0) {enemies.push(new enemy(16*(getRandomInt(13)+1), 16*-2, 16, 8, 0, 120, 5, 5, 4000, 4000, 1))}
	checkInputDown()
...
function startUpdate() {
	if (checkInputDown()&&t.now-t.modeinit>1000) {
		...
		shots = []
		info.score = 0
		info = {score: 0, highscore: 1000, shotsTaken: 0, shotsHit: 0, accuracy: 100}
...
function startDraw() {
	...
	c.ctx.font = font1
	c.ctx.textAlign = "center"
	c.ctx.fillText("MEMORY LEAK", 240*0.5, 320*0.5)
	c.ctx.font = font3
	c.ctx.textAlign = "center"
...
function enemyTimeout() {
	for (i=0;i<enemies.length;i++) {
		enemies[i].life -= t.elapsed
...
function shotOutOfBounds() {
	for (i=0;i<shots.length;i++) {
		if (shots[i].x+shots[i].w*0.5<0 || shots[i].x-shots[i].w*0.5>240 || shots[i].y+shots[i].h*0.5<0 || shots[i].y-shots[i].h*0.5>320) {
			shots.splice(i,1);
		}
	}
}
...
function checkEnemyPlayerCollision() {
	for (i=0;i<enemies.length;i++) {
		var xd = Math.abs(player1.x - enemies[i].x)
		var yd = Math.abs(player1.y - enemies[i].y)
		var xgap = player1.w*0.5 + enemies[i].w*0.5
		var ygap = player1.h*0.5 + enemies[i].h*0.5
		if (xd<xgap && yd<ygap) {
			c.ctx.font = font1
			c.ctx.textAlign = "center"
			c.ctx.fillText("GAME OVER", 240*0.5, 320*0.5)
...
function drawInfo() {
	...
	c.ctx.font = font4
	c.ctx.textAlign = "center"
	c.ctx.fillText(info.highscore, 240*0.5, 16)
